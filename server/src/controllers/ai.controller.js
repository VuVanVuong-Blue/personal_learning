...
Chỉ trả về độ dài tựa đề, không bọc ngoặc phẩy, không giải thích.`;
        ...
        for (const mBlock of rawMilestones) {
            ...
            for (const tBlock of rawTasks) {
                ...
                // Fix: Đảm bảo mỗi mục emoji (🎯 💡 🧠 🚀 🔗) được xuống dòng đúng
                // Gemini đôi khi trả về \n (1 dòng) thay vì \n\n (2 dòng) → marked gộp thành 1 đoạn
                const fixMarkdownSpacing = (md) => {
                    if (!md) return md;
                    return md
                        // Đảm bảo mỗi section emoji bắt đầu bằng paragraph mới
                        .replace(/([^\n])\n(🎯|💡|🧠|🚀|🔗|📌|⚡|✅)/g, '$1\n\n$2')
                        // Đảm bảo header markdown (##, ###) cũng có khoảng cách
                        .replace(/([^\n])\n(#{1,4}\s)/g, '$1\n\n$2')
                        // Đảm bảo bullet list có khoảng cách trước
                        .replace(/([^\n])\n(-\s|\*\s|\d+\.\s)/g, '$1\n\n$2')
                        // Gộp 3+ dòng trống thành 2 dòng (chuẩn hóa)
                        .replace(/\n{3,}/g, '\n\n');
                };

                tasks.push({
                    title: taskTitle,
                    content: marked.parse(fixMarkdownSpacing(taskContentRaw)),
...
// 6. AI QUIZ GENERATOR (Gemini 2.5 Flash — JSON Schema)
export const generateMilestoneQuiz = async (req, res) => {
    try {
        const { milestoneTitle, tasks } = req.body;
        // tasks: [{ title, content }]
        if (!tasks || tasks.length === 0) {
            return res.status(400).json({ message: 'Không có dữ liệu bài học để tạo câu hỏi.' });
        }
        ...
        // Build context from tasks (cap at 8000 chars to avoid token overflow)
        const MAX_CONTENT_CHARS = 8000;
        let studyContext = `Chặng: ${milestoneTitle}\n\nCÁC BÀI HỌC:\n`;
        for (const task of tasks) {
            const textContent = stripHtml(task.content);
            studyContext += `\n--- Bài: ${task.title} ---\n${textContent || '(Chưa có nội dung chi tiết)'}\n`;
            if (studyContext.length > MAX_CONTENT_CHARS) {
                studyContext = studyContext.substring(0, MAX_CONTENT_CHARS) + '\n[... nội dung đã được cắt ngắn ...]';
                break;
            }
        }
        ...
        const prompt = `Bạn là một Giáo viên xuất sắc chuyên thiết kế đề kiểm tra kiến thức.

Dựa vào nội dung bài học sau đây, hãy tạo ra ĐÚNG 5 câu hỏi trắc nghiệm (Multiple Choice) để kiểm tra mức độ hiểu bài của học viên.

YÊU CẦU BẮT BUỘC:
- Mỗi câu hỏi phải có ĐÚNG 4 lựa chọn (A, B, C, D).
- Câu hỏi phải bám sát nội dung bài học, KHÔNG hỏi những điều không có trong tài liệu.
- Phân bố độ khó: 2 câu dễ, 2 câu trung bình, 1 câu nâng cao.
- Lựa chọn sai phải hợp lý (plausible), không quá rõ ràng là sai.
- Giải thích (explanation) phải ngắn gọn, súc tích (1-3 câu), chỉ ra tại sao đáp án đúng là đúng.
- Toàn bộ nội dung phải bằng Tiếng Việt.

NỘI DUNG BÀI HỌC:
${studyContext}

HÃY TRẢ VỀ JSON HỢP LỆ THEO SCHEMA ĐÃ KHAI BÁO.`;

        const quizSchema = {
            description: 'Bộ câu hỏi trắc nghiệm kiểm tra kiến thức',
            type: SchemaType.OBJECT,
            properties: {
                quiz: {
                    type: SchemaType.ARRAY,
                    description: 'Danh sách 5 câu hỏi trắc nghiệm',
                    items: {
                        type: SchemaType.OBJECT,
                        properties: {
                            question: {
                            ...
                            options: {
                                type: SchemaType.ARRAY,
                                description: 'Mảng 4 lựa chọn theo thứ tự A, B, C, D',
                                items: { type: SchemaType.STRING }
                            },
                            correctAnswer: {
                                type: SchemaType.INTEGER,
                                description: 'Index (0-3) của đáp án đúng trong mảng options'
                            },
                            explanation: {
                                type: SchemaType.STRING,
        ...
        if (!quizData.quiz || quizData.quiz.length === 0) {
            return res.status(500).json({ message: 'AI không sinh ra được câu hỏi nào. Vui lòng thử lại.' });
        }

        console.log(`✅ Quiz Generator: Tạo thành công ${quizData.quiz.length} câu hỏi cho chặng "${milestoneTitle}"`);
        res.status(200).json({ success: true, data: quizData.quiz });

    } catch (error) {
        console.error('Quiz Generator Error:', error);
        res.status(500).json({ message: 'Lỗi khi tạo câu hỏi kiểm tra. Vui lòng thử lại.', errorDetail: error.message });
    }
};
