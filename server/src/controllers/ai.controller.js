...
Chỉ trả về độ dài tựa đề, không bọc ngoặc phẩy, không giải thích.`;
        ...
        try {
            ...
            // Xóa lùi từng ký tự từ cuối lên, và thử đóng ngoặc dần dần
            for (let i = repairedStr.length - 1; i >= Math.max(0, repairedStr.length - 1000); i--) {
                const substr = repairedStr.substring(0, i);

                // Các mũi tiêm vá lỗi tiềm năng đắp vào đuôi
                const patches = [
                    '"]}]}', // Đang viết dở content string + đóng task obj + đóng tasks array + đóng milestone obj + đóng chặng array
                    '}]}]}', // Đóng object
                    '}]}',    // Đóng tasks array
                    ']}',     // Đóng milestones array
                    '}'       // Đóng object tổng
                ];

                for (let patch of patches) {
                    try {
                        let potentialFix = substr;
                        // Nếu đang ghép patch dạng string '"]', ta cũng nên kiểm tra xem chỗ bị đứt có bắt đầu bằng nháy kép chưa
                        if (patch.startsWith('"')) {
                            // Đảm bảo không tạo thành chuỗi 3 dấu nháy
                            if (potentialFix.endsWith('"')) potentialFix = potentialFix.slice(0, -1);
                            // Đảm bảo không kết thúc bằng escape
                            if (potentialFix.endsWith('\\')) potentialFix = potentialFix.slice(0, -1);
                        }

                        aiData = JSON.parse(potentialFix + patch);
                        success = true;
                        console.log(`✅ Khôi phục thành công bằng miếng vá: ${patch}`);
                        break;
                    } catch (e) {
                        // Tiếp tục lặp
                    }
